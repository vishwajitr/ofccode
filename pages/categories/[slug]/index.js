...
export async function getServerSideProps({ params }) {
  ...
  const response = await fetch(
    `http://ofccode-api-git-main-sportybruh1990.vercel.app/directPartners/flipkart__offers?q=getCategoryFeed&catName=${cat__name}`
  );
...
//   console.log(ProdURL);
  ...
  const responseProd = await fetch(
    `http://ofccode-api-git-main-sportybruh1990.vercel.app/directPartners/flipkart__offers?q=getProductsFeed&resourceName=${resourceName}&catName=${cat__name}`
  );
